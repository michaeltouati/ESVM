...
#
```
4) If you've added a new feature, let's call it 'my-new-feature', create a directory 'my-new-feature' in 'test-cases/Tests/New-features' and add a typical input deck inside that uses your new feature with parameters such as :
...
for the test to be fast and a text file entitled 'output' containing the terminal output generated by the corresponding simulation using your new feature. For this, you can just type from the directory 'test-cases/Tests/New-features/my-new-feature'
```sh
../../../.././esvm > output
```
Finally, add in the makefile at the section ```## TESTING ##``` (after the line ```TEST_DIR=' Maxwell/Ampere'```) the following line
```sh
