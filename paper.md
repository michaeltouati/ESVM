...
![Two stream instability test case : Stationary electron plasma waves angular frequency \autoref{eq:omega0} and the two-stream instability growth rate \autoref{eq:growth_rate} seeded by the perturbation \autoref{eq:perturbation} as a function of the spatial angular frequency mode $k$.\label{fig:poles}](test-cases/Two-Stream-Instability/poles.png){ width=70% }
