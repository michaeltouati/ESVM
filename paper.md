...
\end{equation}
Performing the same reasoning when also considering the advection of plasma electrons along the $\underline{v_x}$-axis in the phase-space, with bins centered at $\underline{v_x}^\ell = \underline{v_{x,\mathrm{min}}} + (\ell-1 ) \underline{\Delta v}_x$ where $\ell \in \left[ 1,N_{v_x} \right ]$ and $N_{v_x} = 1 + (\underline{v_{x,\mathrm{max}}}-\underline{v_{x,\mathrm{min}}}) / \underline{\Delta v}_x$, in order to compute the Vlasov equation \autoref{eq:vlasov1d1v} with each numerical scheme implemented in ESVM, one finds (sometimes empirically when it is too difficult analytically) that
\begin{equation}
  \label{CFL}
